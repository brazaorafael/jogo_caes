...
# Lista de perguntas + alternativas
perguntas = [
...
def next_q():
    # só avança se já escolheu algo
    if st.session_state.answers[st.session_state.idx] is not None:
        st.session_state.idx += 1

# Barra de progresso
progresso = (st.session_state.idx + 1) / len(perguntas)
...
# Exibe opções e salva a resposta
choice = st.radio(
    "", 
    list(opts.keys()),
...
# Botão Próxima ou Ver Resultado
if st.session_state.idx < len(perguntas) - 1:
    ...
    if st.button("Ver Resultado"):
        ...
        # Exibe o card com nome exato (primeira letra maiúscula)
